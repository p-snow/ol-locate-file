...
#+HTML: <a href="https://melpa.org/#/ol-locate-file"><img src="https://melpa.org/packages/ol-locate-file-badge.svg"></a> <a href="https://stable.melpa.org/#/ol-locate-file"><img src="https://stable.melpa.org/packages/ol-locate-file-badge.svg"></a>

Locate-based Org file link with which users can abbreviate file paths.

* Introduction

~ol-locate-file~ provides a new Org link type that resolves partial file path substrings into full absolute paths using the =locate= command (or an alternative like =plocate=, =find=, or =fd=).

Instead of writing a full absolute path:

#+begin_example
[[lfile:~/data/my-documents/journals/20260707_journal.txt][Daily Journal on 2026-07-07]]
#+end_example
...
** Automated instlattion

This package is available on MELPA. You can install it via Emacs' `package.el` after running `M-x package-refresh-contents`, or define the package using the `use-package` macro.
